...
\begin{eg}[Subset Sum]
	...
	\[ f_{ss}(\vec{a}, \vec{b}) = (\vec{a}, s=\sum_{i=1}^n a_i b_i \bmod N) = (\vec{a}, \langle \vec{a}, \vec{b} \rangle \bmod N) \]

	The inversion game is as follows. We choose $\vec{a}\in \ZZ_N^n, \vec{b}\gets \bit^n$, and give $y = (\vec{a}, s)$ to $\mathcal I$.
	$\mathcal I$ outputs $(\vec{a}^*, \vec{b}^*)$ and wins if $f_{ss}(\vec{a}^*,\vec{b}^*) = y \iff \vec{a}^* = \vec{a}$ and $\langle \vec{a}^*, \vec{b}^* \rangle = s\bmod N$.
\end{eg}
